...
A foundational principle of Cosmos is that the development of blockchain solutions becomes as convenient and accessible as possible. To this end, a variety of tools is available to developers, first and foremost the Cosmos SDK.
...
The Cosmos network focuses on an ecosystem for easy blockchain development that provides customizability and interoperability, establishing a stable universe determined by the same rules that apply to the whole ecosystem equally.

_Click on the hotspots in the image to learn more about the main goals of the Cosmos network._

<H5PComponent :contents="['/h5p/M1-ecosystem-constellation-HS']"></H5PComponent>

The Cosmos SDK is a generalized framework to build secure blockchain applications on Tendermint BFT in Golang (also known as Go). It is a modular framework for application-specific blockchains. The design is based on two major principles: **modularity** and **capability-based security**. The Cosmos SDK was envisioned to be an npm-like framework for secure applications on top of Tendermint. Over time it has become an advanced framework for custom application-specific blockchains.

The ready-built modules of the Cosmos SDK are easy to import, adapt, and use. Developers can additionally create their own modules to introduce specific functionalities. With the growth of the ecosystem, the number of modules grows, facilitating the development of more complex applications.

<ExpansionPanel title="The object-capability model and security in the Cosmos SDK">
...
The default consensus mechanism available when developing with the SDK is [Tendermint Core](https://docs.tendermint.com/master/).

</HighlightBox>

## Alternative blockchain frameworks and SDKs

As the Cosmos SDK is modular, developers can port existing codebases in Go on top of the SDK. This allows developers to build on Cosmos without having to compromise too much on the toolset and environment they use.
...
Ethermint is a software developed to port the EVM into a Cosmos module. It makes scalable, high-throughput, PoS blockchains possible, which are fully compatible with Ethereum and the Cosmos SDK.
...
<ExpansionPanel title="Quick overview: IBC">

The Inter-Blockchain Communication Protocol (IBC) is a common framework for information exchange between blockchains. You will learn more about IBC in the [next section](ADD LINK HERE).

For now, it is enough to know that it exists and it enables seamless interaction between blockchains that want to transfer value (token transfers) and exchange information. It enables the communication between applications that run on separate application-specific blockchains.
...
In addition to the Cosmos SDK itself, there are a variety of related tools for developers to make use of.

### CosmJS

**CosmJS** is a JavaScript library for the Cosmos SDK, and is used to write front-end applications and servers that interact with a blockchain. It is a [powerful tool](https://github.com/cosmos/cosmjs/wiki/What-can-CosmJS-do-for-me%3F), which can be used to create wallets, explorers, IBC relayers, and other decentralized applications (dApps). It is written in TypeScript and can therefore be client or server side.
...
With CosmWasm, smart contracts can run on multiple chains with the help of the Inter-Blockchain Communication Protocol (IBC). It is written as a module to be plugged into the Cosmos SDK and leverages the speed of Wasm and the power of Rust through an actor-model design focused on providing a library.

<ExpansionPanel title="The actor model">

The actor model is a design pattern for reliable, distributed systems, and the pattern underlying CosmWasm smart contracts. Each actor has access to its internal state and can only message other actors through a "dispatcher", which maintains the state and maps addresses to code and storage.
